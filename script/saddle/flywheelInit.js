...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim ars for at once. default 100 (~2.3M gas)
...
let accountRequest = async (network, opts) => {
	...
	let url = `https://api.${stageUrl}aquarius.loan/api/v2/account?min_borrow_value_in_eth[value]=0.00000000000000001&network=${network}&page_size=${pageSize}&page_number=1`;
...
let filterInitialized = async (borrowersByAToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling arsBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let aTokenAddr of Object.keys(borrowersByAToken)) {
		let supplySpeed = await call(Comptroller, 'arsSupplySpeeds', [aTokenAddr]);
		let borrowSpeed = await call(Comptroller, 'arsBorrowSpeeds', [aTokenAddr]);
			...
			for (let borrowerChunk of getChunks(borrowersByAToken[aTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'arsBorrowerIndex',[aTokenAddr, borrower])
					}));
...
let claimArsBatch = async (borrowersByAToken, opts) => {
	for (let aTokenAddr of Object.keys(borrowersByAToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(Comptroller, 'claimArs', [
						chunk,
						[aTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimArsBatch(unInit, opts);
