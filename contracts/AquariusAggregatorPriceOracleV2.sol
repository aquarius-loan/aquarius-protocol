...
contract AquariusAggregatorPriceOracleV2 is PriceOracle {
    using SafeMath for uint256;
    address public admin;

    mapping(address => uint) prices;
    mapping(bytes32 => AggregatorV2V3Interface) internal feeds;
    uint256 public defaultMaxStalePeriod = 86400;
    mapping(address => uint256) public maxStalePeriods;
    ...
    function getChainlinkPrice(AggregatorV2V3Interface feed) internal view returns (uint) {
        ...
        uint256 deltaTime = block.timestamp.sub(updatedAt);

        uint256 maxStalePeriod = maxStalePeriods[address(feed)] > 0 ? maxStalePeriods[address(feed)] : defaultMaxStalePeriod;
    ...
    function setMaxStalePeriod(address feed, uint256 value) external {
        require(msg.sender == admin, "only admin can set max stale period");
        maxStalePeriods[feed] = value;
    }

    function setDefaultMaxStalePeriod(uint256 value) external {
        require(msg.sender == admin, "only admin can set max stale period");
        defaultMaxStalePeriod = value;
    }
