...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV3, GovernorBravoEvents {
    ...
    uint public constant MIN_PROPOSAL_THRESHOLD = 5000000e18; // 5,000,000 Ars

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 10000000e18; //10,000,000 Ars
    ...
    uint public constant quorumVotes = 20000000e18; // 20,000,000 = 2% of Ars
    ...
    /**
      ...
      * @param ars_ The address of the ARS token
      * @param votingPeriod_ The initial voting period
      * @param votingDelay_ The initial voting delay
      * @param proposalThreshold_ The initial proposal threshold
      */
    function initialize(address timelock_, address ars_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_) public {
        require(address(timelock) == address(0), "GovernorBravo::initialize: can only initialize once");
        require(msg.sender == admin, "GovernorBravo::initialize: admin only");
        require(timelock_ != address(0), "GovernorBravo::initialize: invalid timelock address");
        require(ars_ != address(0), "GovernorBravo::initialize: invalid ars address");
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(
            ars.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold 
