...
export interface ATokenMethods {
  ...
  liquidateBorrow(borrower: string, aTokenCollateral: string): Sendable<number>;
  liquidateBorrow(borrower: string, repayAmount: encodedNumber, aTokenCollateral: string): Sendable<number>;
...
export interface ATokenScenarioMethods extends ATokenMethods {
  setTotalBorrows(amount: encodedNumber): Sendable<void>;
  setTotalReserves(amount: encodedNumber): Sendable<void>;
}

export interface AToken extends Contract {
  methods: ATokenMethods;
  name: string;
}

export interface ATokenScenario extends Contract {
  methods: ATokenScenarioMethods;
  name: string;
}
