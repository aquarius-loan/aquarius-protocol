...
async function makeComptroller(opts = {}) {
  ...
  if (kind == 'unitroller-g3') {
    ...
    const maxAssets = etherUnsigned(dfn(opts.maxAssets, 10));
    const liquidationIncentive = etherMantissa(1);
    const arsRate = etherUnsigned(dfn(opts.arsRate, 1e18));
    const arsMarkets = opts.arsMarkets || [];
    const otherMarkets = opts.otherMarkets || [];

    await send(unitroller, '_setPendingImplementation', [comptroller._address]);
    await send(comptroller, '_become', [unitroller._address, arsRate, arsMarkets, otherMarkets]);
    mergeInterface(unitroller, comptroller);
    await send(unitroller, '_setLiquidationIncentive', [liquidationIncentive]);
    await send(unitroller, '_setCloseFactor', [closeFactor]);
    await send(unitroller, '_setMaxAssets', [maxAssets]);
  ...
  if (kind == 'unitroller-g6') {
    ...
    await send(unitroller, '_setCloseFactor', [closeFactor]);
    await send(unitroller, '_setPriceOracle', [priceOracle._address]);
    await send(unitroller, '_setArsRate', [arsRate]);
    await send(unitroller, 'setArsAddress', [ars._address]); // harness only

    return Object.assign(unitroller, { priceOracle, ars });
  }
  ...
  if (kind == 'unitroller') {
    ...
    await send(unitroller, '_setPriceOracle', [priceOracle._address]);
    await send(unitroller, 'setArsAddress', [ars._address]); // harness only
    await send(unitroller, 'harnessSetArsRate', [arsRate]);
...
async function makeAToken(opts = {}) {
  ...
  const symbol = opts.symbol || (kind === 'cether' ? 'aCORE' : 'cOMG');
  ...
  switch (kind) {
    ...
    case 'aars':
      underlying = await deploy('Ars', [opts.arsHolder || root]);
  ...
  if (opts.addArsMarket) {
    await send(comptroller, '_addArsMarket', [aToken._address]);
  }
